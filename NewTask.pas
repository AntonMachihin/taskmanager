...
chr(39)+'Направлен в цех на исполнение'+chr(39)+');';
MyQuery1.SQL.Add(S);
MyQuery1.ExecSQL;
MyQuery1.Close;
//Ведение лога
...
') and (Stat = ''Направлен в цех на исполнение'')';
MyQuery1.SQL.Add(S);
MyQuery1.Open;
ID:=MyQuery1.FieldValues['ID'];
S:= 'Insert into log(idtask, Name, Zapis, Vrem) Values ('+inttostr(ID)+
', '+chr(39)+UserName+chr(39)+', '+chr(39)+'Задача назначена: '
+formlogin.DelQ(edZaglav.Text)+'; Формулировка: '+formlogin.DelQ(memTask.Lines.Text)+'; Назначена на: '+
cmbMaster.Text+'; Дата начала: '+formatdatetime('yyy-mm-dd',Date)+'; Дата выполнения: '+
