...
begin
if ((edMaster.Text = '')and(cmbDateNazn.Text ='Любая')and(cmbDateOkon.Text ='Любая')) then
  begin
...
formOper.SetDBGrid();
  ...
  if cmbDateNazn.Text = 'Выбрать' then
    begin
  ...
  if cmbDateOkon.Text = 'Выбрать' then
    begin
    if (edMaster.Text <> '')or(cmbDateNazn.Text = 'Выбрать') then sdtEnd:=' AND ';
  ...
  if OperRole then S:='SELECT id as ''№'', Stat as ''Статус'', Zaglav as ''Заголовок'', Mast as ''Исполнитель'', Task as ''Задача'', Data_sozd as ''Дата создания'', Data_vyp as ''Дата выполн.'', Comment as ''Комментарий'' FROM zadachi WHERE (Operator = '+chr(39)+UserName+chr(39)+') AND '+res+sdtNazn+sdtEnd;
  if MasterRole then S:='SELECT id as ''№'', Stat as ''Статус'', Zaglav as ''Заголовок'', Operator as ''Назначивший'', Task as ''Задача'', Data_sozd as ''Дата создания'', Data_vyp as ''Дата выполн.'', Comment as ''Комментарий'' FROM zadachi WHERE (Mast = '+chr(39)+UserName+chr(39)+') AND '+res+sdtNazn+sdtEnd;
...
begin
if MasterRole then label1.Caption:='Постановщик';
end;
